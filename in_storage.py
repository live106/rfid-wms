...
class InStorage(QWidget):
    ...
    def showEpcs(self, row):
        ...
        epc_list_table.setColumnCount(5)
        epc_list_table.setHorizontalHeaderLabels(["EPC", "Barcode", "Product Name", "Time", "PickingNo"])
        for i, data in enumerate(epcs):
            if data:
                ...
                pno_item = QTableWidgetItem(data[4])
                ono_item = QTableWidgetItem(data[5])
                ...
                epc_list_table.setItem(i, 4, pno_item)
                epc_list_table.setItem(i, 5, ono_item)
